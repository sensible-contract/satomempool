...
# Build binary output
RUN GOPROXY=https://goproxy.cn,direct GOOS=${GO_OS} GOARCH=${GO_ARCH} go build -o satomempool -ldflags '-s -w' main.go
...
COPY --chown=sato --from=build /build/satomempool /data/satomempool
